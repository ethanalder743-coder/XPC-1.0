...
To send an offer, a manager needs both an allowed management role and exactly one configured team role. They then run `/offer player` without selecting or mentioning a team.

Players must allow direct messages from server members to receive offers. Discord embed sidebars support a static colour only; this bot uses the configured team role colour. Discord does not support animated embed borders.
