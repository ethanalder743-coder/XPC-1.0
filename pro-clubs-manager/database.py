...
class Database:
    ...
    def add_team(self, guild_id: int, name: str, role_id: int) -> None:
        with self.connect() as db:
            db.execute(
                "INSERT INTO teams (guild_id, name, role_id) VALUES (?, ?, ?)",
                (guild_id, name.strip(), role_id),
            )
    ...
    def teams(self, guild_id: int) -> list[sqlite3.Row]:
        with self.connect() as db:
            return list(
                db.execute(
                    "SELECT name, role_id FROM teams WHERE guild_id = ? ORDER BY name",
                    (guild_id,),
                )
            )
    ...
    def team(self, guild_id: int, name: str) -> sqlite3.Row | None:
        with self.connect() as db:
            return db.execute(
                "SELECT name, role_id FROM teams WHERE guild_id = ? AND name = ?",
                (guild_id, name.strip()),
            ).fetchone()

    def configure_logs(self, guild_id: int, signing_id: int, release_id: int) -> None:
        with self.connect() as db:
            db.execute(
                """
                INSERT INTO guild_config (guild_id, signing_channel_id, release_channel_id)
                VALUES (?, ?, ?)
                ON CONFLICT(guild_id) DO UPDATE SET
                    signing_channel_id = excluded.signing_channel_id,
                    release_channel_id = excluded.release_channel_id
                """,
                (guild_id, signing_id, release_id),
            )
